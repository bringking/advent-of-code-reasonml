...
let lineSegments = vectors => {
  Array.fold_left(
    (lineSegments, currentVector) => {
      ...
      let (direction, distance) = currentVector;
      switch (direction) {
      | "U" =>
        Array.append(
          lineSegments,
          [|[|(endX, endY), (endX, endY + distance)|]|],
        )
      | "D" =>
        Array.append(
          lineSegments,
